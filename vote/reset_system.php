...
$submitDataFile = BASE_PATH . '/../submit/data.json'; // 投稿数据文件
...
function resetSubmitData() {
    global $submitDataFile;
    
    if (!file_exists($submitDataFile)) {
        return true; // 文件不存在，无需重置
    }
    
    // 备份原文件
    $backupPath = $submitDataFile . '.backup.' . date('Y-m-d_H-i-s');
    if (!copy($submitDataFile, $backupPath)) {
        throw new Exception('备份投稿数据文件失败，请检查文件权限');
    }
    
    // 读取现有数据
    $data = json_decode(file_get_contents($submitDataFile), true);
    if (!is_array($data)) {
        $data = [];
    }
    
    // 只清空 submissions 和 feedbacks，保留其他配置
    $data['submissions'] = ["all" => [], "approved" => [], "rejected" => []];
    $data['feedbacks'] = [];
    
    if (file_put_contents($submitDataFile, json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT), LOCK_EX) === false) {
        throw new Exception('重置投稿数据失败，无法写入文件');
    }
...
try {
    ...
    switch ($method) {
        case 'POST':
            ...
            // 重置投稿数据
            if (in_array('submit', $resetOptions)) {
                resetSubmitData();
                $results[] = '投稿数据已重置';
            }
        ...
        case 'GET':
            ...
            // 投稿数据状态
            if (file_exists($submitDataFile)) {
                $submitData = json_decode(file_get_contents($submitDataFile), true) ?: [];
                $submissions = $submitData['submissions'] ?? [];
                $feedbacks = $submitData['feedbacks'] ?? [];
                $status['submit'] = [
                    'all_count' => count($submissions['all'] ?? []),
                    'approved_count' => count($submissions['approved'] ?? []),
                    'rejected_count' => count($submissions['rejected'] ?? []),
                    'feedbacks_count' => count($feedbacks)
                ];
            } else {
                $status['submit'] = ['all_count' => 0, 'approved_count' => 0, 'rejected_count' => 0, 'feedbacks_count' => 0];
            }
